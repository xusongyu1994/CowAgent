...
博查、AnySearch、Serply、Tavily 需要单独配置专用密钥；智谱、百度千帆、LinkAI 直接复用对应模型的 API Key，配好模型即同时获得搜索能力。SearXNG 为自托管方案，只需配置实例 URL。Keenable 需手动开启：将 `tools.web_search.keenable_anonymous` 设为 `true`（或在控制台「搜索」面板留空密钥保存）后调用公共接口，按 IP 限流（10 次/秒、1000 次/小时）；配置可选的 `keenable_api_key`（或 `KEENABLE_API_KEY`）后限制解除，且无需开启匿名模式。
